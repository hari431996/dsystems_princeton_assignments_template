...
func (wk *Worker) register(master string) {
	...
	args.Worker = wk.name
	// calling the register function in master through master Server.
...
func RunWorker(MasterAddress string, me string,
	...
	nRPC int, // This argument is used to fail the worker after some tasks.
) {
	debug("RunWorker %s\n", me)
	wk := new(Worker)
	wk.name = me
	...
	for {
		...
		if err == nil {
			...
			wk.nRPC--
			wk.Unlock()
			// This creates a new thread of execution
			// where a function call is made to the approprite function.
			// from an rpc in schedule.
